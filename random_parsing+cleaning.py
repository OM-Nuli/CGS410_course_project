...
def process_all_languages():
    ...
    for filename, lang_code in file_to_lang.items():
        ...
        # 1. Run the batched parser
        results = parse_random_batched(sentences, lang_code, batch_size=128)

        # ==========================================
        # STEP 4: DATA CLEANING
        # Filter out low-quality entries (URLs, 1-word tokens, parsing errors)
        # ==========================================
        cleaned_results = [d for d in results if d["sentence_length"] > 2 and d["avg_dep_length"] > 0]
        
        print(f"🧹 Cleaned data: Kept {len(cleaned_results)} out of {len(results)} valid sentences.")

        # 2. Save the CLEANED results
